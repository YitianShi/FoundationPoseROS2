...
import time
import torch
from tool_new_obj.d_2_pcd import depth_to_point_cloud
...
class PoseEstimationNode(Node):
    def __init__(self, new_file_paths):
        ...
        self.image_sub = self.create_subscription(Image, '/camera_bolt/color/image_raw', self.image_callback, 10)
        self.depth_sub = self.create_subscription(Image, '/camera_bolt/depth/image_raw', self.depth_callback, 10)
        self.info_sub = self.create_subscription(CameraInfo, '/camera_bolt/color/camera_info', self.camera_info_callback, 10)
    ...
    def process_images(self):
        ...
        pcd_img = depth_to_point_cloud(self.depth_image, self.cam_K, pcd_image=True)
        ...
        for idx, data in self.pose_estimations.items():
            ...
            if hasattr(self, 'hand_pose_estimator'):
                torch.set_default_device('cpu')
                visualization_image = self.hand_pose_estimator.estimate_hand_pose_single(color, 
                                                                                         pcd_img = pcd_img,
                                                                                         visualization_img=visualization_image)
...
def main(args=None):
    source_directory = "demo_data/angle_grinder"
